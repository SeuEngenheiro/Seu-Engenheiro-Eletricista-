...
export async function verificarLimitePerguntas(telefone) {
  ...
  if (usuario?.plano === 'pro' || usuario?.plano === 'premium') return { permitido: true, restantes: 999 };
  const hoje = new Date(); hoje.setHours(0,0,0,0);
  const { count } = await supabase.from('perguntas').select('*', { count: 'exact', head: true }).eq('telefone', telefone).gte('enviado_em', hoje.toISOString());
  const usados = count || 0;
  return { permitido: usados < 5, usados, restantes: Math.max(0, 5 - usados) };
}

export async function verificarLimiteFotos(telefone, plano) {
  if (plano === 'premium') return { permitido: true, restantes: 999 };
  if (plano !== 'pro') return { permitido: false, restantes: 0 };
  const hoje = new Date(); hoje.setHours(0,0,0,0);
  const { count } = await supabase.from('fotos').select('*', { count: 'exact', head: true }).eq('telefone', telefone).gte('enviado_em', hoje.toISOString());
  const usados = count || 0;
  return { permitido: usados < 20, usados, restantes: Math.max(0, 20 - usados) };
}
