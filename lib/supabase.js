...
/**
 * Retorna data do próximo reset (dia 1 do MÊS SEGUINTE) formatada DD/MM.
 * Usada pra comunicar ao usuário quando o saldo volta a 20.
 *
 * IMPORTANTE: ordem das chamadas evita o bug de Date overflow no JS.
 *   d = new Date('2026-05-31');
 *   d.setMonth(d.getMonth()+1);  // tenta 31/06 → overflow vira 01/07 ❌
 * Solução: zerar pra dia 1 ANTES de pular o mês.
 */
export function proximoResetMensal() {
  const d = new Date();
  d.setDate(1);                // primeiro vai pro dia 1 do mês atual
  d.setMonth(d.getMonth() + 1); // agora pula pro próximo mês com segurança
