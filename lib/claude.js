...
export async function analisarFoto(telefone, imageBase64, mimeType, plano = 'pro') {
  try {
    const historico = getHistorico(telefone);
    const contextoPlano = `[CONTEXTO: Usuário com plano ${plano.toUpperCase()} — analise esta foto de instalação elétrica e forneça orientação técnica precisa baseada na NBR 5410]`;
      ...
      messages: [
        ...
        ...historico,
        {
          ...
          content: [
            ...
            {
              type: 'text',
              text: 'Analise esta foto de instalação elétrica. Identifique problemas, riscos e recomendações técnicas. ' + contextoPlano
            }
          ]
        }
      ]
    ...
    const novoHistorico = [
      ...historico,
      { role: 'user', content: '[Foto enviada para análise]' },
      { role: 'assistant', content: resposta }
    ].slice(-10);
    ...
    throw new Error('Erro ao analisar a foto. Tente novamente.');
  }
}
