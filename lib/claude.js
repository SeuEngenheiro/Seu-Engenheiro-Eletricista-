...
// Timeout máximo da chamada OpenAI — alvo: resposta total < 5s
// 4500ms deixa ~500ms pra envio Z-API + Supabase finais
...
export async function chamarClaude(telefone, mensagem, plano = 'gratis') {
  ...
  try {
    ...
    // AbortController garante que a OpenAI não passe do timeout
    ...
    try {
      response = await client.chat.completions.create({
        model: 'gpt-5-mini',
        // 800 cobre 95% das respostas (típica: 150-300 tokens) com folga.
        // Reduzir teto de geração diminui tail latency em 30-50%.
        max_completion_tokens: 800,
        // 'minimal' (em vez de 'low') desliga reasoning interno do gpt-5.
        // Reduz TTFB de ~3-8s para ~500ms-1s. Trade-off: menos "pensar"
        // antes de responder — pra cálculos diretos e citação de norma,
        // o prompt já dá estrutura suficiente. Se notarmos perda de
        // qualidade, voltamos pra 'low'.
    ...
    console.log(
      `[TIMING] hist=${tHistorico - t0}ms llm=${tLLMEnd - tLLMStart}ms total=${Date.now() - t0}ms`
    );
